...
while True:
    ...
    while i<len(user_guess):
        if user_guess[i]==chosen_word[i]:
            clue=clue+f'[{user_guess[i]}]'
        elif user_guess[i] in chosen_word:
            clue=clue+f'({user_guess[i]})'
        else:
            clue=clue+f'{user_guess[i]}'
        i=i+1
