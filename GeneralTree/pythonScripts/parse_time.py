...
def main():
    ...
    with open('V:\\R29SP2\\Perf_Impact_Automation\\timeFiles.list','r') as f:
        f_rd=f.readlines()
        time_files=[x.strip() for x in f_rd]
    for file in time_files:
        tree_data=parse(file)
        print(len(tree_data))
        #+----{ Write tree_data to file }----+
        #     with open('V:\\R29SP2\\Perf_Impact_Automation\\!101474!Case3ApiRayFire.scm.time.txt','w') as f:
        #         for data in tree_data:
        #             print(len(data))
        #             f.write(str(data)+'\n')
        #+----{ Print tree_data levelize }----+
        #     for data in tree_data:
        #         if int(data[0])>0:
        #             print('\t'*int(data[0]),str(data[1]),str(data[2]))
        #+----{ Get maximum and rayfire_entitylist timings }----+
        perf_culprit=[]
        rayfire_entitylist=[]
        flag=False
        max_time=0.00
        ...
        #print(perf_culprit)
        #print(rayfire_entitylist)
#         with open('V:\\R29SP2\\Perf_Impact_Automation\\res.txt','w') as f:
#             f.write('+----{ Performance Culprit }----+\n')
#             f.write(str(perf_culprit)+'\n') 
#             f.write('+----{ rayfire_entitylist data }----+\n')
...
#                 f.write(str(data)+'\n')
        ...
        if perf_culprit[1]!='rayfire_entitylist':
            with open('V:\\R29SP2\\Perf_Impact_Automation\\Overall_res.txt','a') as f:
